    ...
    <div className="mx-auto max-w-4xl p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <div>
          ...
          <select value={currentSquad} onChange={e=>onChangeSquad(e.target.value)} className="border rounded p-2 w-full">
            {squads.map(s => <option key={s.id} value={s.id}>{s.nombre}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm">Rival</label>
          <select value={opponentId} onChange={e=>setOpponentId(e.target.value)} className="border rounded p-2 w-full">
      ...
      {isOpen && form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 bg-white rounded-xl shadow-xl w-full max-w-3xl p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea className="border rounded p-2 w-full min-h-[100px]" value={form.notas || ""} onChange={e=>setForm({ ...form!, notas: e.target.value })} />
              </div>
            </div>
  ...
  return (
    <div>
      <div className="flex gap-2 mb-2">
        <input className="border rounded p-2 flex-1" placeholder={`Añadir a ${label.toLowerCase()}`} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} />
